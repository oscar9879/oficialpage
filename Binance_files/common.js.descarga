...
function shouldShowDisclaimerTopBar() {
  ...
  // only show on these url rules
  const shouldShowByRules = SHOW_RULES.some(rule => window.location.href.includes(rule))

  const { region = '', locale = '', disclaimerConfig = {} } = bnvelidate || {}
  const { enable = false } = disclaimerConfig

  if (shouldShowByRules && region.toLowerCase() === TARGET_REGION && locale === TARGET_LNG) {
    return true
  }

  return enable && region.toLowerCase() === TARGET_REGION && locale === TARGET_LNG
}
...
function shouldRedirectByGateway(skipCheck) {
  // if skipCheck is true, the redirection should be handled by FE, return false
  if (skipCheck) {
    console.log('shouldRedirectByGateway, skipCheck is true, will check the redirection by FE')
    return false
  }

  var list = ['binancezh.info', 'qa1fdg', 'devfdg', 'localhost']
  // if current href is in the list, the redirection should be handled by gateway, return true
  if (list.some(item => window.location.href.includes(item))) {
    console.log('shouldRedirectByGateway, current domain is in the list, should redirect by gateway')
    return true
  }

  // passed from backend config
  if (window._GLOBALJS_REDIRECTION_DISABLE === 1) {
    console.log('shouldRedirectByGateway, backend config disabled FE redirection, should redirect by gateway')
    return true
  }

  console.log('shouldRedirectByGateway, current domain is not in the list, will check the redirection by FE')
  return false
}
...
function fetchRedirectionDisableConfig() {
  return new Promise(function (resolve, reject) {
    if (window._GLOBALJS_REDIRECTION_DISABLE === true) {
      resolve()
    } else {
      window._GLOBALJS_REDIRECTION_DISABLE = 'loading'
      bnvelidate
        .postBNHTTP('/bapi/apex/v1/public/apex/marketing/web/commonConfig', {
          keys: ['com.globalRedirection.disable'],
        })
        .then(function (res) {
          if (res && res.data && res.data.res) {
            var isDisabled = res.data.res['com.globalRedirection.disable']
            window._GLOBALJS_REDIRECTION_DISABLE = isDisabled || false
            resolve()
          }
        })
        .catch(function (err) {
          console.log(err)
          window._GLOBALJS_REDIRECTION_DISABLE = false
          resolve()
        })
    }
  })
...
function handleRedirect(url, isReplace) {
  if (shouldRedirectByGateway()) {
    return
  }
...
function checkIsKzUser(isLoggedIn) {
  if (isLoggedIn) {
    return getCookie('BNC-Location') === 'KZ'
  }
  return false
}

function handleKzRedirect(isLoggedIn) {
  const isKz = checkIsKzUser(isLoggedIn)
  const URL_QUERY = '?kzDialogShow=show'

  if (!isKz) return
  const { locale = '' } = bnvelidate || {}

  if (['en-KZ', 'ru-KZ', 'kk-KZ'].includes(locale)) return

  if (locale === 'en') {
    handleRedirect(getUrl(`en-KZ${URL_QUERY}`))
    return
  }
  if (locale === 'kz') {
    handleRedirect(getUrl(`kk-KZ${URL_QUERY}`))
    return
  }
  handleRedirect(getUrl(`ru-KZ${URL_QUERY}`))
}

function handleHkRedirect() {
  const { region = '', locale = '' } = bnvelidate || {}

  if (!region || region.toLowerCase() !== 'hk') return

  if (locale === 'zh-TC') {
    handleRedirect(window.location.pathname.replace('zh-TC', 'en'), true)
    return true
  }
}
...
function pollAndWaitCondition(callback, condition, interval = 100, timeout = 5000) {
  var timer = null
  var startTime = Date.now()
  function poll() {
    if (condition()) {
      console.log('pollCondition wait success')
      clearInterval(timer)
    } else if (Date.now() - startTime > timeout) {
      console.log('pollCondition timeout')
      callback()
      clearInterval(timer)
    }
  }
...
/**
 * This function is used to check if the user needs to be redirected because of UK regulation
 * Now this case has been moved to the gateway in prod, only leave the comment here for recent reference
 * will be removed in the future
 */

// function callComplianceActionCheck() {
//   function executor() {
//     console.log('callComplianceActionCheck redirect from common.js')
//     bnvelidate
//       .postBNHTTP('/bapi/apex/v1/friendly/apex/marketing/complianceActionCheck', {
//         requestLink: window.location.href,
//       })
//       .then(function (res) {
//         if (res && res.success && res.data && !res.data.pass) {
//           var actionType = res.data.actionType || ''
//           var clientType = res.data.clientType || ''
//           var actionInfo = res.data.actionInfo || {}

//           // only handle web redirect
//           if (!(clientType === 'web' && actionInfo.actionUrl)) return

//           if ('redirect_rel' === actionType) {
//             // handle redirect relative path, if actionUrl start with '/'
//             if (actionInfo.actionUrl.startsWith('/')) {
//               handleRedirect(bnvelidate.replaceUrl('https://www.%suffixOrigin%' + actionInfo.actionUrl), true)
//               return
//             }

//             // only handle locale part for window.location.href
//             var newLocale = actionInfo.actionUrl
//             // if (bnvelidate.locale === newLocale) return
//             var newRedirectUrl = window.location.href.replace('/' + bnvelidate.locale, '/' + newLocale)
//             handleRedirect(newRedirectUrl, true)
//           }

//           if ('redirect_abs' === actionType) {
//             handleRedirect(actionInfo.actionUrl, true)
//           }
//         }
//       })
//       .catch(function (err) {
//         console.error('callComplianceActionCheck error = ', err)
//       })
//   }
...
//     executor,
//     function () {
//       return window._EXTENSION_REDIRECT_FLAG
//     },
//     100,
//     5000,
//   )
...
;(function () {
  var main = function () {
    ...
    const { isLoading, initialFrom } = bnvelidate || {}
    const shouldHandleLoginCompliance =
      typeof isLoading === 'boolean' && initialFrom !== 'loader' && initialFrom !== 'standalone'

    if (shouldHandleLoginCompliance) {
      let commonJsTimer = setInterval(function () {
    ...
    function main() {
      const { isLoggedIn } = bnvelidate || {}

      handleKzRedirect(isLoggedIn)
    }
    ...
    function hkPopup() {
      var { region } = bnvelidate || {}
      if (region.toLowerCase() !== 'hk') return
  ...
  fetchRedirectionDisableConfig().then(function () {
    main()
  })
})()
